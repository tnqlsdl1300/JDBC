...
, fk_bookisbn varchar2(30) -- 참조
, isrent  number(2)
, constraint PK_hw_bookid_bookid primary key(bookid)
, constraint FK_hw_bookid_bookisbn foreign key(fk_bookisbn) references hw_isbn(bookisbn) on delete cascade    -- isbn이 사라지면 해당 책을 반입?하지 않는거기 때문에 개별 책들의 정보도 삭제
);
...
(fk_userid   varchar2(30)  -- 참조
, fk_bookid  varchar2(30)  -- 참조
, rentalday   date default sysdate
, returnday  date default sysdate + 7
, arrears  number(5)
, constraint FK_hw_rental_userid foreign key(fk_userid) references hw_member(userid)   -- userid와 bookid는 대여중인 회원이 있을 때 도서 정보와 회원정보를 삭제하면 안되므로 옵션을 주지 않음
, constraint FK_hw_rental_bookid foreign key(fk_bookid) references hw_bookid(bookid)   -- 위와 같음
);
